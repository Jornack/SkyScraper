...
public class ExcelGenerator extends JSONFileReader {
		...
		try{
			...
			while(isDataAvailable()){
				...
				if (!json.isNull("poorSignalLevel")){
					Number numPsl = new Number(COLUMN_PSL, row, json.getInt("poorSignalLevel"));
				...
				if (!json.isNull("eSense")){
					...
					Number numAtt 	= new Number(COLUMN_ATT 	 ,row,esense.getInt("attention"));
					Number numMed 	= new Number(COLUMN_MED 	 ,row,esense.getInt("meditation"));
				...
				if (!json.isNull("eegPower")){
					...
					Number numDelta 	= new Number(COLUMN_DELTA 	 ,row,eegPower.getInt("delta"));
					Number numTheta 	= new Number(COLUMN_THETA 	 ,row,eegPower.getInt("theta"));
					Number numLowAlpha 	= new Number(COLUMN_LOALPHA  ,row,eegPower.getInt("lowAlpha"));
					Number numHiAlpha 	= new Number(COLUMN_HIALPHA  ,row,eegPower.getInt("highAlpha"));
					Number numLowBeta 	= new Number(COLUMN_LOBETA   ,row,eegPower.getInt("lowBeta"));
					Number numHiBeta 	= new Number(COLUMN_HIBETA   ,row,eegPower.getInt("highBeta"));
					Number numLoGamma 	= new Number(COLUMN_LOGAMMA  ,row,eegPower.getInt("lowGamma"));
					Number numHiGamma 	= new Number(COLUMN_HIGAMMA  ,row,eegPower.getInt("highGamma"));
	...
	public WritableWorkbook getWorkbook(TimeSeriesCollection eSense, TimeSeriesCollection eegPower) throws IOException{
		WritableWorkbook workbook = Workbook.createWorkbook(this.target);
		WritableSheet sheet = workbook.createSheet(this.sheetName, 0); 
		...
		try {
			...
			if ( eegPower.getSeriesCount() > 0){
				...
				for (int i = 0 ; i <eegPower.getSeries(0).getTimePeriods().size(); i++) {
					Millisecond period = (Millisecond) eegPower.getSeries(0).getTimePeriod(i);
					Number numMs = new Number		( COLUMN_MS,i+1,period.getFirstMillisecond());
					System.out.println("number0=:" +numMs.getValue() );     
					Number numAtt = null;
					Number numMed = null;
					if (eSense.getSeries(0).getValue(period)!= null){
						 numAtt = new Number( COLUMN_ATT,i+1,(Double) eSense.getSeries(0).getValue(period));   
						 numMed = new Number( COLUMN_MED,i+1,(Double) eSense.getSeries(1).getValue(period));   
					...
					Number numDelta 		= new Number		(COLUMN_DELTA 	 ,i+1,(Double) eegPower.getSeries(0).getValue(period));          
					Number numTheta 		= new Number		(COLUMN_THETA 	 ,i+1,(Double) eegPower.getSeries(1).getValue(period));
					Number numLowAlpha  = new Number		(COLUMN_LOALPHA  ,i+1,(Double) eegPower.getSeries(2).getValue(period));
					Number numHiAlpha   = new Number		(COLUMN_HIALPHA  ,i+1,(Double) eegPower.getSeries(3).getValue(period));
					Number numLowBeta   = new Number		(COLUMN_LOBETA   ,i+1,(Double) eegPower.getSeries(4).getValue(period));
					Number numHiBeta 	  = new Number		(COLUMN_HIBETA   ,i+1,(Double) eegPower.getSeries(5).getValue(period));
					Number numLoGamma   = new Number		(COLUMN_LOGAMMA  ,i+1,(Double) eegPower.getSeries(6).getValue(period));
					Number numHiGamma    = new Number	(COLUMN_HIGAMMA  ,i+1,(Double) eegPower.getSeries(7).getValue(period));
...
//		attentionSeries      .add(ms,2); 
	    ...
	    File source = new File("d:\\Program Files (x86)\\Eclipse IDE for Java Developers\\eclipse\\workspace\\SkyScraper.JH\\skyskraper.20120211 09.50.5312.json" );
	    ExcelGenerator me2 = new ExcelGenerator(source, new File("me2.xls"), "Foo2 Session");
