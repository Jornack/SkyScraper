...
public class LineChartGenerator
{
    ...
    public JFreeChart generate(File file)
    {
        ...
        boolean ignorePSl = PreferenceManager.getPreferences().getBoolean(
                PreferenceManager.CHART_GENERATION_IGNORE_PSL, false);
        boolean ignoreESense = PreferenceManager.getPreferences().getBoolean(
                PreferenceManager.CHART_GENERATION_IGNORE_ESENSE, false);
        boolean ignoreEEGPwr = PreferenceManager.getPreferences().getBoolean(
                PreferenceManager.CHART_GENERATION_IGNORE_EEGPWR, false);
        if (!ignorePSl)
        {
        ...
        while (reader.isDataAvailable())
        {
            ...
            try
            {
                ...
                if (!json.isNull("poorSignalLevel"))
                {
                    ...
                    poorSignalLevelSeries.addOrUpdate(ms, (double) json.getInt(
                            "poorSignalLevel"));
                }
                /*
                ...
                if (!json.isNull("eSense"))
                {
                    ...
                    attentionSeries.addOrUpdate(ms, (double) esense.getInt(
                            "attention"));
                    meditationSeries.addOrUpdate(ms, (double) esense.getInt(
                            "meditation"));
                ...
                if (!json.isNull("eegPower"))
                {
                    ...
                    deltaSeries.addOrUpdate(ms, (double) eegPower.
                            getInt("delta"));
                    thetaSeries.addOrUpdate(ms, (double) eegPower.
                            getInt("theta"));
                    lowAlphaSeries.addOrUpdate(ms, (double) eegPower.getInt(
                            "lowAlpha"));
                    highAlphaSeries.addOrUpdate(ms, (double) eegPower.getInt(
                            "highAlpha"));
                    lowBetaSeries.addOrUpdate(ms, (double) eegPower.getInt(
                            "lowBeta"));
                    highBetaSeries.addOrUpdate(ms, (double) eegPower.getInt(
                            "highBeta"));
                    lowGammaSeries.addOrUpdate(ms, (double) eegPower.getInt(
                            "lowGamma"));
                    highGamaSeries.addOrUpdate(ms, (double) eegPower.getInt(
                            "highGamma"));
        ...
        try
        {
            ...
            int width = PreferenceManager.getPreferences().getInt(
                    PreferenceManager.CHART_GENERATION_WIDTH, 4096);
            int height = PreferenceManager.getPreferences().getInt(
                    PreferenceManager.CHART_GENERATION_HEIGHT, 512);
            String format = PreferenceManager.getPreferences().get(
                    PreferenceManager.CHART_GENERATION_FORMAT, "PNG");
            resizeChart(chart, first, last);
            if (format.equals("PNG"))
            {
                ChartUtilities.saveChartAsPNG(new File(file.getAbsoluteFile()
                        + ".png"), chart, width, height);
            } else
            {
                ChartUtilities.saveChartAsJPEG(new File(file.getAbsoluteFile()
                        + ".jpg"), 1.0f, chart, width, height);
            }
        } catch (IOException e)
        {
            // TODO Auto-generated catch block
